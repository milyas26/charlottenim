...
import { useBookPurchaseDetail, useUploadBookPaymentProof } from "@/lib/api/books";
import { useAuth } from "@/contexts/AuthContext";
import LoginDialog from "@/components/LoginDialog";
import BottomNav from "@/components/layout/BottomNav";
import { Loader2, ArrowLeft, Upload, CheckCircle, XCircle, Clock, AlertCircle, Copy, MessageCircle } from "lucide-react";
...
export default function PesananPage({
  ...
  const { data: purchase, isLoading, refetch } = useBookPurchaseDetail(purchaseId);
  const uploadMutation = useUploadBookPaymentProof();
  ...
  const formatAddress = () => {
    ...
    return [
      ...
      [shippingAddress.districtName, shippingAddress.regencyName, shippingAddress.provinceName, shippingAddress.postalCode]
        .filter(Boolean)
        .join(", "),
    ]
      .filter(Boolean)
  ...
  const whatsappUrl = purchase
    ? `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(
        `Halo admin Charlottenim! Saya mau tanya ongkos kirim untuk pesanan:\n\n📖 Judul: ${purchase.bookTitle}\n🆔 ID Pesanan: ${purchase.id}\n📦 Alamat:\n${formatAddress()}\n\nMohon info ongkirnya ya. Terima kasih!`
      )}`
    : "";
    ...
    return (
      <div className="min-h-screen bg-[var(--background)] flex flex-col">
        <div className="flex-1 max-w-[480px] mx-auto w-full px-4 flex items-center justify-center pb-24">
          <p className="text-sm" style={{ color: "var(--muted)" }}>Pesanan tidak ditemukan.</p>
        </div>
        <BottomNav />
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] flex flex-col">
      <div className="flex-1 max-w-[480px] mx-auto w-full px-4 pb-24">
        ...
        <h1
          className="text-lg font-bold text-center mb-2 font-[family-name:var(--font-display)]"
          style={{ color: "var(--foreground)" }}
        >
          {purchase.bookTitle}
        </h1>
        <p className="text-xs text-center mb-6" style={{ color: "var(--muted)" }}>
          ID: {purchase.id.slice(0, 12)}...
        </p>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <p className="text-sm font-semibold" style={{ color: "var(--foreground)" }}>
              Total Harga Buku (Belum Termasuk Ongkir)
            </p>
            <p
            ...
            <div className="mb-4">
              ...
              >
                <p style={{ color: "var(--foreground)" }}>
                  <span className="font-medium">{shippingAddress.recipientName}</span> • {shippingAddress.phone}
                </p>
                <p style={{ color: "var(--muted)" }}>
                  {shippingAddress.address}
                </p>
                <p style={{ color: "var(--muted)" }}>
                  {[shippingAddress.districtName, shippingAddress.regencyName, shippingAddress.provinceName].filter(Boolean).join(", ")} {shippingAddress.postalCode}
                </p>
              </div>
            </div>
          )}
          ...
          <a
            ...
            className="flex items-center justify-center gap-2 w-full py-3 px-6 rounded-xl text-sm font-semibold transition-opacity hover:opacity-90 tap-feedback mb-4"
            style={{ backgroundColor: "#25D366", color: "white" }}
          >
            <MessageCircle className="size-4" />
            Tanya Ongkos Kirim via WhatsApp
          </a>

          <div className="gold-rule max-w-[80px] my-4" />

          <p className="text-xs mb-3" style={{ color: "var(--muted)" }}>
            Transfer sejumlah total harga buku (+ ongkir) ke rekening:
          </p>
            ...
            {!purchase.paymentProofUrl ? (
              ...
              >
                ...
                <p className="text-xs mb-4" style={{ color: "var(--muted)" }}>
                  Setelah transfer (harga buku + ongkir), upload screenshot/foto bukti transfer di sini. Admin akan memverifikasi pembayaran Anda.
                </p>
                ...
                {user ? (
                  <label
                    className="flex items-center justify-center gap-2 py-3 px-6 rounded-xl text-white font-semibold text-sm transition-opacity tap-feedback cursor-pointer hover:opacity-90"
              ...
              >
                ...
                {purchase.paymentProofUrl && (
                  <>
                    ...
                    <label
                      className="flex items-center justify-center gap-2 py-2 px-4 rounded-xl text-xs font-semibold transition-opacity hover:opacity-90 tap-feedback cursor-pointer"
        ...
        {purchase.status === "PAID" && (
          ...
          >
            ...
            <p className="text-xs mb-4" style={{ color: "var(--muted)" }}>
              Pesanan Anda akan segera diproses. Admin akan menghubungi Anda untuk informasi pengiriman.
            </p>
          </div>
        )}
